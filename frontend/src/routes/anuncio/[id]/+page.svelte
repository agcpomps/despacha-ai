<script lang="ts">
	import { featureListing, unfeatureListing, bumpListing } from '$lib/remote/listings.remote';
	...
	// O anúncio vem do load server-side (+page.server.ts) para que as meta
	// tags de partilha saiam no HTML inicial.
	let { data } = $props();
	const item = $derived(data.listing);
...
<div class="grid gap-6 lg:grid-cols-[1fr_360px]">
	...
	<aside class="space-y-4">
		...
		<div class="rounded-2xl bg-amber-50 p-4 text-xs leading-relaxed text-amber-800">
			<strong>Dica de segurança:</strong> encontra-te com o vendedor num local público e verifica o artigo
			antes de pagar.
		</div>
